...
def render_tab_refine():
    ...
    with col_ref:
        ...
        if st.button("후처리 교정 실행", width="stretch"):
    ...
    with col_down:
        ...
        if rj_code:
            ...
            if os.path.exists(target_dldata_dir) and os.path.isdir(target_dldata_dir):
                ...
                if st.button("💾 DLdata 폴더에 번역본 및 백업 자동 내보내기", width="stretch", type="primary"):
                    try:
        ...
        if st.button("💾 프로젝트 백업 폴더에 즉시 저장", width="stretch"):
            try:
        ...
        if st.button("📝 지정 경로에 파일로 저장", width="stretch"):
