import json
import re

from core.progress_store import (
    load_chunk_summary,
    load_image_note,
    save_chunk_summary,
    save_image_note,
)


def _model_family(model, processor) -> str:
    config = getattr(model, "config", None)
    model_type = str(getattr(config, "model_type", "") or "").lower()
    archs = [str(a).lower() for a in (getattr(config, "architectures", []) or [])]
    template = str(getattr(processor, "chat_template", "") or "")
    tokenizer = getattr(processor, "tokenizer", None)
    template += str(getattr(tokenizer, "chat_template", "") or "")

    if "qwen" in model_type or any("qwen" in a for a in archs) or "qwen" in template.lower():
        return "qwen"
    if "gemma" in model_type or any("gemma" in a for a in archs) or "gemma" in template.lower():
        return "gemma"
    if hasattr(processor, "image_processor"):
        return "vlm"
    return "text"


def _json_rules(model, processor) -> str:
    if _model_family(model, processor) == "qwen":
        return """[JSON 형식 엄격 준수 규칙]
1. 먼저 <think> 태그 내에서 필요한 분석 단계를 자유롭게 서술해도 좋습니다.
2. 태그를 닫은 후에는 반드시 위에 지정된 JSON 형식으로만 결과물을 작성하세요.
3. JSON 앞뒤에 마크다운 코드펜스(```json), 인삿말, 설명문을 붙이지 마세요.
4. 모든 문자열 값 내부에서 일반 큰따옴표를 쓰지 말고 작은따옴표 또는 한국어 따옴표를 사용하세요."""

    return """[JSON 형식 엄격 준수 규칙]
1. 오직 JSON 객체 하나만 출력하세요.
2. <think> 태그, 분석 과정, 인삿말, 설명문, 마크다운 코드펜스(```json)를 절대 출력하지 마세요.
3. 첫 글자는 반드시 { 이고 마지막 글자는 반드시 } 이어야 합니다.
4. 모든 문자열 값 내부에서 일반 큰따옴표를 쓰지 말고 작은따옴표 또는 한국어 따옴표를 사용하세요."""


def _json_correction_rules(model, processor) -> str:
    if _model_family(model, processor) == "qwen":
        return """[JSON 형식 엄격 준수 규칙]
1. 먼저 <think> 태그 내에서 필요한 분석 단계를 자유롭게 서술해도 좋습니다.
2. 태그를 닫은 후에는 반드시 아래 예시된 JSON 형식으로만 결과물을 작성하세요.
3. 다른 설명이나 마크다운 백틱은 일절 생략하십시오."""

    return """[JSON 형식 엄격 준수 규칙]
1. 오직 문법적으로 유효한 JSON 객체 하나만 출력하세요.
2. <think> 태그, 분석 과정, 설명, 마크다운 백틱은 일절 생략하십시오.
3. 첫 글자는 반드시 { 이고 마지막 글자는 반드시 } 이어야 합니다."""


def _generate_text(
    model,
    processor,
    prompt: str,
    image_paths: list[str] | None = None,
    max_tokens: int = 1024,
    temp: float = 0.1,
    repetition_penalty: float = 1.1,
    force_json: bool = False,
) -> str:
    from core.utils import has_repetition

    image_paths = image_paths or []
    
    # 명시적 이미지 태그 처리 (VLM 토큰 유출 및 템플릿 오류 예방)
    prompt_content = prompt
    if len(image_paths) > 0 and "<image>" not in prompt:
        prompt_content = "<image>\n" + prompt

    try:
        from core.openrouter import OpenRouterClient
        if isinstance(model, OpenRouterClient):
            import os
            import base64
            content = []
            if image_paths:
                for img_path in image_paths:
                    if os.path.exists(img_path):
                        with open(img_path, "rb") as f:
                            b64_data = base64.b64encode(f.read()).decode("utf-8")
                        mime = "image/png" if img_path.lower().endswith(".png") else "image/jpeg"
                        content.append({
                            "type": "image_url",
                            "image_url": {
                                "url": f"data:{mime};base64,{b64_data}"
                            }
                        })
            if content:
                content.append({"type": "text", "text": prompt_content})
                messages = [{"role": "user", "content": content}]
            else:
                messages = [{"role": "user", "content": prompt_content}]
                
            max_retries = 3
            for retry in range(max_retries):
                current_temp = temp
                if retry > 0:
                    current_temp = min(0.7, temp + 0.2 * retry)
                try:
                    generator = model.generate_stream(messages, temp=current_temp, max_tokens=max_tokens)
                    output = ""
                    for response in generator:
                        output += response.text
                        if has_repetition(output):
                            from core.utils import strip_repetition
                            output = strip_repetition(output)
                            break
                    from core.document import clean_markdown
                    return clean_markdown(output)
                except Exception as e:
                    if retry == max_retries - 1:
                        raise e
                    print(f"[OpenRouter RETRY] Error occurred, retrying: {e}")
    except ImportError:
        pass

    # VLM vs Text-only check
    is_vlm_model = hasattr(processor, "image_processor")

    if is_vlm_model:
        from mlx_vlm.generate import stream_generate
        from mlx_vlm.prompt_utils import apply_chat_template

        messages = [{"role": "user", "content": prompt_content}]
        formatted_prompt = apply_chat_template(
            processor,
            model.config,
            messages,
            num_images=len(image_paths),
            num_audios=0,
        )
    else:
        from mlx_lm import stream_generate
        from mlx_lm.sample_utils import make_sampler, make_logits_processors
        messages = [{"role": "user", "content": prompt_content}]
        formatted_prompt = processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        if force_json:
            if formatted_prompt.endswith("<think>\n"):
                formatted_prompt = formatted_prompt[:-8]
            elif formatted_prompt.endswith("<think>"):
                formatted_prompt = formatted_prompt[:-7]
            if not formatted_prompt.endswith("{"):
                formatted_prompt += "{"

    max_retries = 3
    for retry in range(max_retries):
        current_temp = temp
        current_penalty = repetition_penalty
        if retry > 0:
            current_temp = min(0.7, temp + 0.2 * retry)
            current_penalty = repetition_penalty + 0.15 * retry
            print(f"[RETRY] Loop detected, retrying {retry}/{max_retries-1} with temp={current_temp:.2f}, penalty={current_penalty:.2f}")

        output = ""
        if is_vlm_model:
            generator = stream_generate(
                model,
                processor,
                prompt=formatted_prompt,
                image=image_paths or None,
                temp=current_temp,
                max_tokens=max_tokens,
                repetition_penalty=current_penalty,
                repetition_context_size=100,
                seed=42,
            )
        else:
            sampler = make_sampler(temp=current_temp)
            logits_processors = make_logits_processors(
                repetition_penalty=current_penalty,
                repetition_context_size=100
            )
            generator = stream_generate(
                model,
                processor,
                prompt=formatted_prompt,
                max_tokens=max_tokens,
                sampler=sampler,
                logits_processors=logits_processors,
            )

        loop_detected = False
        for response in generator:
            output += response.text
            if has_repetition(output):
                print(f"[WARNING] Repetition loop detected! Stopping stream.")
                loop_detected = True
                break

        if force_json and not is_vlm_model and not output.strip().startswith("{"):
            output = "{" + output

        if not loop_detected:
            return output

    return output


from core.model_manager import clear_mlx_cache as _clear_mlx_cache


def _parse_json_response(response: str) -> dict:
    if not response:
        return {}

    cleaned_resp = response.strip()
    
    # Strip any leading/trailing tags like <|im_start|>, assistant, etc.
    cleaned_resp = re.sub(r"^(<\|im_start\|>|<\|im_end\|>|assistant|user|\s)+", "", cleaned_resp)
    cleaned_resp = re.sub(r"(<\|im_start\|>|<\|im_end\|>|assistant|user|\s)+$", "", cleaned_resp)

    # Strip thinking process tag block
    cleaned_resp = re.sub(r"<think>.*?</think>", "", cleaned_resp, flags=re.DOTALL)
    cleaned_resp = re.sub(r"<think>.*", "", cleaned_resp, flags=re.DOTALL)

    fenced_match = re.search(r"```(?:json|JSON)?\s*(.*?)\s*```", cleaned_resp, re.DOTALL)
    if fenced_match:
        cleaned_resp = fenced_match.group(1).strip()

    # 1. Clean block markers
    json_match = re.search(r"\{.*\}", cleaned_resp, re.DOTALL)
    json_str = json_match.group(0) if json_match else cleaned_resp

    # Prepend/append braces only after code fences and surrounding chatter are removed.
    if not json_str.startswith("{") and '"' in json_str and ":" in json_str:
        json_str = "{" + json_str
    if not json_str.endswith("}") and '"' in json_str and ":" in json_str:
        json_str = json_str + "}"

    # 2. Fix unescaped double quotes inside string values and list items first
    lines = json_str.split("\n")
    cleaned_lines = []
    
    for line in lines:
        line_strip = line.strip()
        if not line_strip:
            continue
            
        # Fix missing closing quote on keys: "key: ", -> "key": "",
        m = re.match(r'^(\s*)"([a-zA-Z0-9_\-\s/]+):\s*"(,\s*|)$', line)
        if m:
            indent = m.group(1) or ""
            key = m.group(2)
            suffix = m.group(3) or ""
            cleaned_lines.append(f'{indent}"{key}": ""{suffix}')
            continue

        # Standard double quote escaping inside values
        # 1) Key-value pair string: "key": "value"
        kv_match = re.match(r'^(\s*"[a-zA-Z0-9_\-\s/]+"\s*:\s*")(.*)("\s*,?\s*)$', line)
        if kv_match:
            prefix, content, suffix = kv_match.groups()
            clean_content = content.replace('\\"', '"')
            escaped_content = clean_content.replace('"', '\\"')
            line = prefix + escaped_content + suffix
            line_strip = line.strip()
            
        # 2) List item string: "value", or "value"
        elif re.match(r'^(\s*")(.*)("\s*,?\s*)$', line):
            list_match = re.match(r'^(\s*")(.*)("\s*,?\s*)$', line)
            prefix, content, suffix = list_match.groups()
            if prefix and content and suffix:
                if content.strip() not in ("", "[", "]", "{", "}", ","):
                    clean_content = content.replace('\\"', '"')
                    escaped_content = clean_content.replace('"', '\\"')
                    line = prefix + escaped_content + suffix
                    line_strip = line.strip()
                
        # Fix truncated/unclosed quotes at the end of the line
        quotes_count = line_strip.count('"') - line_strip.count('\\"')
        if quotes_count % 2 != 0:
            if line_strip.endswith(','):
                line_strip = line_strip[:-1].rstrip() + '",'
            else:
                line_strip = line_strip + '"'
                
        cleaned_lines.append(line_strip)
        
    json_str = "\n".join(cleaned_lines)

    # 3. Fix keys that are missing closing double quote before colon, e.g. "key: "value"
    json_str = re.sub(r'"([a-zA-Z0-9_\-\s/]+):\s*"', r'"\1": "', json_str)

    # 4. Fix keys without quotes at all, e.g. { key: "value" or , key: "value"
    json_str = re.sub(r'([{,]\s*)([a-zA-Z0-9_\-\s/]+)\s*:', r'\1"\2":', json_str)

    # 5. Remove trailing commas before closing braces/brackets
    json_str = re.sub(r',\s*([\]}])', r'\1', json_str)

    # 6. Standard valid escape processing
    valid_escape_pattern = re.compile(r'(\\["\\/bfnrt]|\\u[0-9a-fA-F]{4})|\\')

    def fix_escape(match):
        if match.group(1):
            return match.group(1)
        return "\\\\"

    json_str = valid_escape_pattern.sub(fix_escape, json_str)
    
    # 7. Stack-based bracket/brace balancing
    stack = []
    for char in json_str:
        if char == '{':
            stack.append('}')
        elif char == '[':
            stack.append(']')
        elif char == '}':
            if stack and stack[-1] == '}':
                stack.pop()
        elif char == ']':
            if stack and stack[-1] == ']':
                stack.pop()
                
    while stack:
        close_token = stack.pop()
        json_str += "\n" + close_token
        
    try:
        return json.loads(json_str, strict=False)
    except json.JSONDecodeError as e:
        try:
            cleaned = re.sub(r"'\s*:", r'":', json_str)
            cleaned = re.sub(r":\s*'", r':"', cleaned)
            cleaned = re.sub(r"([{,]\s*)'", r'\1"', cleaned)
            cleaned = re.sub(r"'\s*([,}])", r'"\1', cleaned)
            return json.loads(cleaned, strict=False)
        except Exception:
            raise e


def analyze_images(model, processor, image_paths: list[str] | None = None) -> str:
    """
    모든 소개 이미지를 한 장씩 VLM에 통과시켜 텍스트 분석 노트로 압축합니다.
    한 번에 여러 이미지를 넣지 않아 Metal 메모리 피크를 낮춥니다.
    """
    if not image_paths:
        return ""

    if not hasattr(processor, "image_processor"):
        print("[WARNING] Text-only model loaded. Skipping image analysis.")
        return ""

    try:
        from core.openrouter import OpenRouterClient
        if isinstance(model, OpenRouterClient):
            return ""
    except ImportError:
        pass

    notes = []
    for idx, image_path in enumerate(image_paths, start=1):
        cached_note = load_image_note(image_path)
        if cached_note:
            notes.append(cached_note)
            continue

        prompt = """
ASMR 작품 소개 이미지 1장을 분석해 주세요.
번역 페르소나와 대본 요약에 도움이 되는 정보만 간결하게 추출하세요.

[추출 항목]
- 캐릭터 외형, 표정, 자세, 분위기
- 이미지 안에 보이는 제목/트랙/설정/관계성/키워드 텍스트
- 화자와 청자의 관계를 추정할 단서
- 말투나 번역 어조에 반영할 단서

설명은 한국어 bullet 형태로만 작성하세요.
"""
        try:
            note = _generate_text(
                model,
                processor,
                prompt,
                image_paths=[image_path],
                max_tokens=600,
                temp=0.1,
            ).strip()
            note_record = f"[이미지 {idx}: {image_path}]\n{note}"
            save_image_note(image_path, note_record)
            notes.append(note_record)
        finally:
            _clear_mlx_cache()

    return "\n\n".join(notes)


def analyze_persona(model, processor, metadata_text: str, script_preview: str, image_paths: list[str] = None) -> dict:
    """
    메타데이터, 대본 일부, 소개 이미지 분석 노트를 기반으로 페르소나 정보(어조, 관계, 상황, 핵심 규칙)만 반환합니다.
    """
    image_notes = analyze_images(model, processor, image_paths)
    image_section = f"\n[소개 이미지 분석 노트]\n{image_notes}\n" if image_notes else ""
    json_rules = _json_rules(model, processor)

    prompt = f"""
ASMR 대본의 설명란 텍스트(메타데이터), 대본 본문 일부, 소개 이미지 분석 노트를 종합하여 상황극 번역에 필요한 캐릭터 페르소나 정보만 추출해주세요. (만약 대본에 등장하는 주요 화자가 2명 이상인 경우, 각 화자들의 페르소나 정보를 모두 빠짐없이 추출해 내야 합니다.)
결과는 반드시 아래의 JSON 형식으로만 작성해야 하며, 다른 설명이나 인삿말은 생략하세요.

[JSON 형식]
{{
  "tone": "캐릭터의 말투와 어조 (만약 등장하는 화자가 2명 이상인 경우, 각 화자별 말투를 '화자A: 어조 / 화자B: 어조'와 같이 명시하여 각각 적어주세요. 예: 히로인: 부드러운 반말)",
  "relationship": "화자와 청자의 관계 (화자가 여러 명이면 각각의 관계를 구분해서 적어주세요. 예: 히로인A-청자: 오래된 연인, 히로인B-청자: 직장 동료)",
  "situation": "전체 배경 상황 및 스토리 맥락 요약 (예: 주인공이 청자를 간호하며 위로해주는 상황)",
  "key_rules": [
    "각 화자별 번역 규칙 또는 공통 규칙 (예: '히로인A의 대사는 반드시 존댓말로 번역', '히로인B의 대사는 반말로 어리광 부리듯 번역' 등 최대 5개)"
  ]
}}

{json_rules}

[입력 데이터]
메타데이터:
{metadata_text}
{image_section}
대본 본문 일부:
{script_preview}
"""
    response = ""
    try:
        response = _generate_text(model, processor, prompt, max_tokens=800, temp=0.1, force_json=True)
        return _parse_json_response(response)
    except Exception as e:
        print(f"[WARNING] Initial Persona JSON parsing failed: {e}")
        try:
            correction_prompt = f"""{_json_correction_rules(model, processor)}

[오류 결과물]
{response}

[출력 대상 JSON 구조 예시]
{{
  "tone": "말투",
  "relationship": "관계",
  "situation": "상황",
  "key_rules": [
    "규칙 1",
    "규칙 2"
  ]
}}"""
            response = _generate_text(model, processor, correction_prompt, max_tokens=800, temp=0.1, force_json=True)
            return _parse_json_response(response)
        except Exception as e2:
            print(f"[ERROR] Persona JSON self-correction failed: {e2}")
            print(f"[DEBUG] Raw Corrected LLM Response:\n{response}")
            return {
                "tone": "일반적인 상황극 말투",
                "relationship": "상황극 캐릭터와 청자",
                "situation": "상황극 대본의 배경 상황",
                "key_rules": [
                    "대본의 맥락을 살려 자연스럽게 번역해주세요.",
                    "지시문 및 타임스탬프 형태를 유지해주세요.",
                ]
            }
    finally:
        _clear_mlx_cache()


def analyze_glossary(model, processor, metadata_text: str, script_preview: str, image_paths: list[str] = None, master_glossary: list = None) -> dict:
    """
    대본과 메타데이터에서 새로운 용어집(Word Mapping)만 단독 추출하여 반환합니다.
    """
    image_notes = analyze_images(model, processor, image_paths)
    image_section = f"\n[소개 이미지 분석 노트]\n{image_notes}\n" if image_notes else ""
    json_rules = _json_rules(model, processor)

    master_section = ""
    if master_glossary:
        # 단어장의 전체 정보를 다 넣으면 컨텍스트가 너무 비대해져 로컬 실행 시 OOM(메모리 부족)을 유발하므로, 원어(Source) 키만 추출하여 조밀하게 전달합니다.
        existing_keys = [item.get("원어 (Source)", "") for item in master_glossary if item.get("원어 (Source)")]
        # 빈 문자열 제외 및 중복 제거
        existing_keys = sorted(list(set([k.strip() for k in existing_keys if k.strip()])))
        
        # 목록을 쉼표로 구분하여 가독성 높이고 토큰을 극소화함
        existing_keys_str = ", ".join(existing_keys)
        
        master_section = f"""
[기존 단어장에 등록된 원어 목록 (중복 추출 방지용)]
{existing_keys_str}

[점진적 추출 규칙]
- 위 기존 단어장 목록에 이미 존재하는 단어는 결과의 'glossary' 목록에서 제외해 주십시오. (중복 추출 방지)
- 기존 단어장 목록에 존재하지 않는 이번 대본에 새로 등장하는 '새로운 고유 용어', '호칭', '상황 맥락적 중요 뉘앙스 표현'들만 찾아내어 'glossary'에 추가해 주십시오.
- 단, 기존 단어장에 이미 있더라도 이번 대본에서 완전히 다른 새로운 뉘앙스나 한국어 번역 옵션으로 매칭되어야 하는 단어라면 설명과 함께 포함해도 좋습니다.
"""

    prompt = f"""
ASMR 대본의 설명란 텍스트(메타데이터), 대본 본문 일부, 소개 이미지 분석 노트를 종합하여 상황극 번역에 필요한 주요 고유 명사/호칭/의성어/의태어/성적 표현 등의 용어집(Glossary)만 추출해주세요.
결과는 반드시 아래의 JSON 형식으로만 작성해야 하며, 다른 설명이나 인삿말은 생략하세요.
{master_section}
[JSON 형식]
{{
  "glossary": [
    {{"source": "대본 내 자주 등장하거나 번역 고정이 필요한 주요 호칭/단어 원어", "target": "제안할 번역어", "context": "단어의 상세 설명이나 뉘앙스"}},
    {{"source": "Onii-chan", "target": "오빠", "context": "상대 화자를 부르는 대표적인 호칭"}}
  ]
}}

{json_rules}

[입력 데이터]
메타데이터:
{metadata_text}
{image_section}
대본 본문 일부:
{script_preview}
"""

    response = ""
    try:
        response = _generate_text(model, processor, prompt, max_tokens=1200, temp=0.1, force_json=True)
        return _parse_json_response(response)
    except Exception as e:
        print(f"[WARNING] Initial Glossary JSON parsing failed: {e}")
        print(f"[DEBUG] Attempting self-correction loop...")
        try:
            correction_prompt = f"""{_json_correction_rules(model, processor)}

[오류 결과물]
{response}

[출력 대상 JSON 구조 예시]
{{
  "glossary": [
    {{"source": "원어", "target": "번역어", "context": "설명"}}
  ]
}}"""
            response = _generate_text(model, processor, correction_prompt, max_tokens=1200, temp=0.1, force_json=True)
            return _parse_json_response(response)
        except Exception as e2:
            print(f"[ERROR] Glossary JSON self-correction failed: {e2}")
            print(f"[DEBUG] Raw Corrected LLM Response:\n{response}")
            return {
                "glossary": []
            }
    finally:
        _clear_mlx_cache()


def analyze_script_summary(
    model,
    processor,
    metadata_text: str,
    script_preview: str,
    image_paths: list[str] = None,
    file_name: str = "script.txt",
    chunks: list[str] | None = None,
) -> dict:
    """
    대본에서 고르게 샘플링한 소수의 씬(또는 대본의 시작/중간/끝 내용)만 분석하여
    VRAM 부하와 모델 호출 횟수를 획기적으로 줄인 대본 요약을 반환합니다.
    """
    image_notes = analyze_images(model, processor, image_paths)
    image_section = f"\n[소개 이미지 분석 노트]\n{image_notes}\n" if image_notes else ""
    json_rules = _json_rules(model, processor)
    
    # 77개 청크를 전부 돌려 요약하지 않고, 전체 대본에서 균등하게 5개 구간을 샘플링하여 결합
    sampled_text = ""
    if chunks:
        total_chunks = len(chunks)
        if total_chunks <= 5:
            sampled_text = "\n\n".join(chunks)
        else:
            # 고르게 5개 구간 추출 (도입부, 25%, 50%, 75%, 결말부)
            indices = [0, int(total_chunks * 0.25), int(total_chunks * 0.5), int(total_chunks * 0.75), total_chunks - 1]
            indices = sorted(list(set(indices))) # 중복 제거 및 정렬
            sampled_blocks = []
            for i in indices:
                if i < total_chunks:
                    sampled_blocks.append(f"[대본 구간 {i + 1} / {total_chunks}]\n{chunks[i][:600]}")
            sampled_text = "\n\n".join(sampled_blocks)
    else:
        sampled_text = script_preview[:3000]

    prompt = f"""
ASMR 대본의 설명란 텍스트, 대본 본문 중 추출된 주요 장면(도입/전개/위기/결말 구간) 샘플, 그리고 소개 이미지 분석 노트를 종합하여 전체 대본의 상황 및 스토리 요약을 작성해주세요.
결과는 반드시 아래 JSON 형식으로만 작성해야 합니다. 다른 인삿말이나 부연 설명은 일절 출력하지 마십시오.

[JSON 형식]
{{
  "speaker_name": "화자(상황극 주인공)의 이름이나 주요 호칭",
  "listener_role": "청자(듣는 사람)의 역할이나 주요 호칭",
  "situation": "전체 상황극의 배경 상황 및 상세 설정 요약",
  "story": "도입-전개-결말에 따른 이야기 흐름 및 감정 변화를 정리한 상세 줄거리"
}}

{json_rules}

[입력 데이터]
메타데이터:
{metadata_text[:1500]}
{image_section}
대본 샘플 장면들:
{sampled_text}
"""

    response = ""
    try:
        response = _generate_text(model, processor, prompt, max_tokens=1024, temp=0.2, force_json=True)
        return _parse_json_response(response)
    except Exception as e:
        print(f"[WARNING] Script Summary JSON parsing failed: {e}")
        # 자가 복구 루프 시도
        try:
            correction_prompt = f"""
{_json_correction_rules(model, processor)}

[오류 결과물]
{response}

[출력 대상 JSON 구조 예시]
{{
  "speaker_name": "화자 이름",
  "listener_role": "청자 역할",
  "situation": "배경 상황",
  "story": "상세 줄거리"
}}
"""
            response = _generate_text(model, processor, correction_prompt, max_tokens=1024, temp=0.1, force_json=True)
            return _parse_json_response(response)
        except Exception as e2:
            print(f"[ERROR] Summary JSON self-correction failed: {e2}")
            return {
                "speaker_name": "미분석",
                "listener_role": "미분석",
                "situation": "대본의 정보가 부족하거나 파싱에 실패했습니다.",
                "story": "대본 요약에 실패했습니다.",
            }
    finally:
        _clear_mlx_cache()


def generate_image_generation_prompt(
    model,
    processor,
    metadata_text: str,
    script_preview: str,
    persona: dict,
    script_summary: dict,
    image_paths: list[str] = None,
) -> str:
    image_notes = analyze_images(model, processor, image_paths)
    key_rules = "\n".join(f"- {rule}" for rule in persona.get("key_rules", []))
    prompt = f"""
당신은 이미지 생성 모델(SDXL, Midjourney 등)용 전문 프롬프트 디렉터입니다.
ASMR 작품의 소개 이미지 분석, 대본 요약, 페르소나를 바탕으로 새로운 고품질 일러스트 대표 이미지를 만들기 위한 상세한 프롬프트를 작성하세요.

[작성 규칙]
1. Positive prompt는 매우 구체적이고 예술적이어야 합니다. 캐릭터의 외형(헤어스타일, 눈빛, 표정 등), 복장, 자세, 분위기에 매칭되는 조명 효과, 상세한 배경 소품, 카메라 앵글, 그리고 전체적인 화풍/스타일까지 꼼꼼하게 묘사하는 풍부한 영문 프롬프트(단어/구 절 형태 및 완성형 문장 혼용)로 작성해주세요.
2. Negative prompt는 절대 작성하지 마십시오. ("Negative prompt:" 항목도 완전히 제거하세요).
3. 결과물은 오직 아래 두 개의 항목으로만 작성해서 출력해야 합니다.

[출력 형식]
Positive prompt:
(여기에 영어 이미지 생성 프롬프트를 세부적으로 채워 넣으세요. 마스터피스, 고품질 일러스트 스타일 관련 태그들도 풍부하게 추가)

Korean brief:
(한국어로 3문장 이내의 이미지 연출 및 시각적 구도 의도 설명)

[페르소나]
- 말투/어조: {persona.get('tone', '')}
- 관계: {persona.get('relationship', '')}
{key_rules}

[대본 요약]
- 화자: {script_summary.get('speaker_name', '')}
- 청자: {script_summary.get('listener_role', '')}
- 상황: {script_summary.get('situation', '')}
- 스토리: {script_summary.get('story', '')[:2000]}

[소개 이미지 분석 노트]
{image_notes}

[메타데이터]
{metadata_text[:2000]}

[대본 일부]
{script_preview[:2500]}
"""
    try:
        return _generate_text(model, processor, prompt, max_tokens=700, temp=0.3).strip()
    finally:
        _clear_mlx_cache()


def classify_proper_nouns(model, processor, glossary_items: list[dict]) -> list[str]:
    """
    용어집의 단어 리스트를 받아서, 고유명사(인물명, 캐릭터명, 지명, 조직명, 고유 명칭 등)인 원어 단어를 판별해 반환합니다.
    """
    if not glossary_items:
        return []
    
    words = [item.get("원어 (Source)", "") for item in glossary_items if item.get("원어 (Source)")]
    words = [w for w in words if w.strip()]
    if not words:
        return []
        
    prompt = f"""당신은 텍스트 내 단어들을 분석하는 한국어 언어학 전문가입니다.
주어진 단어 목록을 분석하여, **고유명사**(사람 이름, 캐릭터 이름, 지명, 조직/단체 이름, 작품 내에서만 쓰이는 고유한 개념/명칭)인 단어들을 식별해 주세요.
일반적인 명사나 대명사, 일반적인 상황극 행동 묘사, 일반 단어는 고유명사에서 제외해야 합니다.

[분석할 단어 목록]
{json.dumps(words, ensure_ascii=False, indent=2)}

반드시 아래 JSON 형식으로만 결과를 반환하십시오. 다른 설명글은 일절 작성하지 마십시오.
```json
{{
  "proper_nouns": [
    "고유명사인_원어_단어1",
    "고유명사인_원어_단어2"
  ]
}}
```"""
    try:
        response = _generate_text(model, processor, prompt, max_tokens=1024, temp=0.1)
        data = _parse_json_response(response)
        return data.get("proper_nouns", [])
    except Exception as e:
        print(f"[classify_proper_nouns] 에러 발생: {e}. raw response: {response}")
        # fallback: 텍스트 내 매칭 단어 반환
        found = []
        for word in words:
            if word.lower() in response.lower():
                found.append(word)
        return found
    finally:
        _clear_mlx_cache()


def generate_suggestion_chips(model, processor, script_preview: str) -> list[str]:
    """
    대본 정보를 바탕으로 사용자가 캐릭터에게 건네기 좋은 추천 대화/상황극 시작 대사 4개를 생성합니다.
    """
    prompt = f"""당신은 ASMR 상황극 및 대본 분석가입니다.
주어진 대본 샘플을 철저히 읽고, 해당 대본 속에서 실제로 일어난 구체적인 사건, 대화 내용, 또는 캐릭터의 대사/행동을 직접 언급하거나 질문하는 맞춤형 대화 선택지(유저용 멘트/행동 묘사)를 4개 추천해 주세요.

[작성 규칙]
1. 일반적이고 뻔한 ASMR 멘트(예: "오늘 하루 어땠어?", "너랑 있으니 좋다")는 절대 배제하고, 반드시 주어진 대본에서 일어난 구체적인 일이나 대사 내용을 기반으로 대화를 이어나가는 내용이어야 합니다.
2. 예시: 대본에서 캐릭터가 코코아를 끓여줬거나, 귀를 파주었거나, 화를 냈거나, 특정한 약속을 했다면 "아까 네가 끓여준 코코아 정말 따뜻하고 맛있었어. 어떻게 끓인 거야?", "아까 나한테 화내서 깜짝 놀랐어. 아직 화 안 풀렸어?" 등 대본의 실제 사건을 직접 언급해야 합니다.
3. 각 대화 선택지는 한 문장 내외로 하며, 사용자가 캐릭터에게 직접 건네는 말이나 행동이어야 합니다.
4. ASMR 상황극 분위기에 어울리게 유저의 행동이나 태도를 나타내는 지시문 `[...]`이 대사 앞이나 중간에 자연스럽게 섞여있도록 작성해 주세요.
5. 당연히 한국어로만 작성해야 합니다. 대본이 일본어/영어 등 원어일 경우에도, 대본 내용을 정확히 파악하여 한국어로 유효한 추천 대화 칩을 만드세요.

[대본 샘플 정보]
{script_preview}

반드시 아래 JSON 형식으로만 답하십시오. 다른 설명은 일절 배제하십시오.
```json
{{
  "suggestions": [
    "대본 사건 기반 유저 추천 발화 1",
    "대본 사건 기반 유저 추천 발화 2",
    "대본 사건 기반 유저 추천 발화 3",
    "대본 사건 기반 유저 추천 발화 4"
  ]
}}
```"""
    try:
        response = _generate_text(model, processor, prompt, max_tokens=512, temp=0.2)
        data = _parse_json_response(response)
        return data.get("suggestions", [])[:4]
    except Exception as e:
        print(f"[generate_suggestion_chips] 에러 발생: {e}")
        # 기본 프리셋
        return [
            "[조심스럽게 옆에 앉으며] 오늘 하루는 어땠어?",
            "[가만히 눈을 맞추며] 네가 해주는 이야기를 듣고 싶어.",
            "[머리를 쓰다듬어 달라는 듯이] 나 조금 피곤한데, 같이 있어줄래?",
            "[미소를 지으며] 너랑 이렇게 단둘이 이야기하니까 정말 좋다."
        ]
    finally:
        _clear_mlx_cache()
